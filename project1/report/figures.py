...
os.system('LU 3 LU')
os.system('gen 6 gen')
os.system('symm 6 symm')
os.system('simple 6 simple')
'''
...
for j in range(0,3):
	file = np.loadtxt('benchmark/'+files[i]+str(3-j)+'.dat', unpack=True)
...
exact = np.loadtxt('benchmark/simple6.dat', unpack=True)
