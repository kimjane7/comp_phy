...
for i in range(0,5):
	...
	plt.plot(file[1],file[2],linewidth=2,label=labels[i],color=colors[i])

plt.legend(loc=1, shadow=True)
plt.xlabel(r'$x$', fontsize=16, weight='normal', family='serif')
plt.ylabel(r'$y$', fontsize=16, weight='normal', family='serif')
plt.title(r'Approximate Orbit of Earth (Forward Euler Method)', fontsize=16, weight='normal', family='serif')
plt.grid()
